...
n_dist = []
for m,n in matches:
    m_dist.append(m.distance)
    n_dist.append(n.distance)
    if m.distance < 0.8*n.distance:
        good.append(m)
    elif m.distance > 0.99 * n.distance:
        bad.append(m)
        bad_n.append(n)
#print('m_distances', m_dist)
#print('n_distances', n_dist)

bad_m_n = list(zip(bad, bad_n))
bad_m_n.sort(key=lambda x: x[0].distance / x[1].distance)
print('last m dist:', bad_m_n[0][-1].distance )
print('last n dist:', bad_m_n[1][-1].distance )
bad_m_sorted, _ = list(zip(*bad_m_n))

src_pts_bad = np.float32([ kp1[m.queryIdx].pt for m in bad_m_sorted ]).reshape(-1,1,2)
dst_pts_bad = np.float32([ kp2[m.trainIdx].pt for m in bad_m_sorted ]).reshape(-1,1,2)

M_bad, mask_bad = cv.findHomography(src_pts_bad, dst_pts_bad, cv.RANSAC,5.0)
matchesMask_bad = mask_bad.ravel().tolist()
print("good", len(good))
print("bad", len(bad))
print('last_good_OG:', good[-1])
#print('good_OG:', good)
good[-1] = bad_m_sorted[-1]
#print('good_new:', good)
print('last_good:', good[-1], '| last_bad:', bad_m_sorted[-1])
...
draw_params_1 = dict(matchColor = (0,255,0), # draw matches in green color
                   singlePointColor = None,
                   matchesMask = matchesMask, # draw only inliers
                   flags = 2)
draw_params_2 = dict(matchColor = (255,0,0), # draw matches in green color
                   singlePointColor = None,
                   matchesMask = matchesMask_bad, # draw only inliers
                   flags = 2)
