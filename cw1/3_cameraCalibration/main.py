...
# Set to True to undistort the test image.
TEST = False
def calibration(data_path: str):
    """Given the path to the input data,
        estimate the camera parameters.

        :param data_path: path to the input data.
            The input data must contain pictures
            taken by the same camera. Each
            picture must contain a 6x8 checkerboard.
        :return: A tuple containing the following
            intrinsic camera matrix,
            new camera matrix,
            distortion parameters,
            rotation vector of each input image
            translation vector of each input image
            region of interest of each input image
    """
...
if __name__ == "__main__":
    mtx, newcameramtx, dist, rvecs, tvecs, roi = calibration()
